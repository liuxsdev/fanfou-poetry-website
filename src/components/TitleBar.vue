<template>
  <div class="title-bar pr-4">
    <div class="control flex space-x-2 items-center">
      <div class="user">
        <span v-if="!isLogined" class="w-4 h-4 hidden">
          <span @click="goAuth">登录</span>
        </span>
        <p v-else>
          <img class="useravatar w-4 h-4" v-bind:src="userinfo.avatar" alt="" />
          <span class="username">{{ userinfo.name }}</span>
          <span class="username" @click="logout">退出</span>
        </p>
      </div>

      <span @click="pre" title="上一首" class="w-4 h-4">
        <Icon :color="ctrl_bottom_color"><AngleLeft /></Icon>
      </span>
      <span @click="next" title="下一首" class="w-4 h-4">
        <Icon :color="ctrl_bottom_color"><AngleRight /></Icon>
      </span>
      <span @click="random" title="随机" class="w-4 h-4">
        <Icon :color="ctrl_bottom_color"><ArrowSync12Regular /></Icon>
      </span>
      <span @click="font_toggle" title="加载字体" class="w-4 h-4 hidden">
